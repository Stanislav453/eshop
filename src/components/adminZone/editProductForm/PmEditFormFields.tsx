...
import { CategoryOptions, type PmEditFormValues } from "../../../type";

type PmEditFormFieldsProps = {
  onCancel: () => void;
};

export const PmEditFormFields = ({ onCancel }: PmEditFormFieldsProps) => {
  const { isSubmitting } = useFormikContext<PmEditFormValues>();
  ...
  return (
    <Form className="flex flex-col gap-2 bg-blue-100 p-3 w-full">
      ...
      <TextInput label="Slug" name="slug" type="text" />
